...
function animate() {
    ...
    // Primero renderizar la vista principal
    renderer.setViewport(0, 0, window.innerWidth, window.innerHeight);
    renderer.render(scene, camera);
    renderer.clearDepth(); 
    const size = Math.min(window.innerWidth, window.innerHeight) / 4; // tamaño de la vista miniatura
    renderer.setViewport(0, window.innerHeight - size, size, size); // posiciona la vista miniatura en la esquina superior izquierda
    renderer.render(scene, orthoCamera);
    
    // Luego renderizar la vista miniatura en la esquina superior izquierda
